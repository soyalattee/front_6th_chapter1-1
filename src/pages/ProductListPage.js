const LoadingSkeleton = (count = 4) => {
  return Array.from({ length: count }, (_, index) => {
    return `
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden animate-pulse" key="${index}">
          ...
          <div class="flex items-center space-x-2">
            ...
            <button id="cart-icon-btn" class="relative p-2 text-gray-700 hover:text-gray-900 transition-colors">
              ...
              ${
                state.cart.length > 0
                  ? `<span
                class="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">${state.cart.length}</span>`
                  : ""
              }
            </button>
          </div>
      ...
      <div class="mb-6">
        ...
        ${
          state.pagination.hasNext
            ? `<div id="infinity-sentinel" style="height: 1px;"></div>`
...
export const ProductListPage = (state) => {
  return ProductListLoaded(state);
};
